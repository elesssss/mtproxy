...
def init_config():
    ...
    if args.config:
        ...
        # 使用传统方式支持额外参数
        conf_dict = {}
        conf_dict.setdefault("PORT", [443])
        if isinstance(conf_dict["PORT"], int):
            conf_dict["PORT"] = [conf_dict["PORT"]]
...
def init_ip_info():
    ...
    IPV4_URL1 = "http://ifconfig.me"
    IPV4_URL2 = "http://api64.ipify.org"

    IPV6_URL1 = "http://ifconfig.me"
    IPV6_URL2 = "http://api64.ipify.org"
    ...
    # 如果返回的 IPv6 地址实际上是 IPv4 地址，则将其设为 None
...
def print_tg_info():
    ...
    if not config.MY_DOMAIN:
        ...
        if not ip_addrs:
            ip_addrs = ["YOUR_IP"]  # 如果未获取到 IP，使用默认值
    else:
    ...
    for user, secret in sorted(config.USERS.items(), key=lambda x: x[0]):
        for ip in ip_addrs:
            for port in config.PORT:  # 遍历所有端口
                ...
                if config.MODES["tls"]:
                    ...
                    params = {"server": ip, "port": port, "secret": tls_secret}
                    params_encodeded = urllib.parse.urlencode(params, safe=':')
...
def create_servers(loop):
    ...
    for port in config.PORT:
        ...
        if config.LISTEN_ADDR_IPV4:
            ...
            # print_err(f"Listening IPv4 on port {port}")
        ...
        if config.LISTEN_ADDR_IPV6 and socket.has_ipv6:
            ...
            servers.append(loop.run_until_complete(task))
            # print_err(f"Listening IPv6 on port {port}")
    ...
    if config.LISTEN_UNIX_SOCK and has_unix:
        ...
        task = asyncio.start_unix_server(
            handle_client_wrapper,
            config.LISTEN_UNIX_SOCK,
            limit=get_to_tg_bufsize()
        )
        servers.append(loop.run_until_complete(task))
        os.chmod(config.LISTEN_UNIX_SOCK, 0o666)
        print_err(f"Listening on Unix socket: {config.LISTEN_UNIX_SOCK}")
    ...
    if config.METRICS_PORT is not None:
        if config.METRICS_LISTEN_ADDR_IPV4:
            task = asyncio.start_server(
                handle_metrics,
                config.METRICS_LISTEN_ADDR_IPV4,
                config.METRICS_PORT
            )
            servers.append(loop.run_until_complete(task))
            print_err(f"Metrics listening IPv4 on port {config.METRICS_PORT}")

        if config.METRICS_LISTEN_ADDR_IPV6 and socket.has_ipv6:
            task = asyncio.start_server(
                handle_metrics,
                config.METRICS_LISTEN_ADDR_IPV6,
                config.METRICS_PORT
            )
            servers.append(loop.run_until_complete(task))
            print_err(f"Metrics listening IPv6 on port {config.METRICS_PORT}")
